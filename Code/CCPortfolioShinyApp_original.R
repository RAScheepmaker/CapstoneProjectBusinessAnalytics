...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("ReMCCO - Recommend Me Credit Cards Optimally"),
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            # TO BE REPLACED WITH CUSTOM BUDGET:
            sliderInput(inputId = "income",
                        "Income for Average Budget:",
                        min = 15000,
                        max = 350000,
                        value = 94000),
        ...
        # Show a plot of the optimal portfolio distribution
        mainPanel(
...
server <- function(input, output) {
    ...
    output$budget_office_supplies <- renderUI({
      ...
      waterfall(values = portfolio()$marginal_benefit, labels = portfolio()$cards,
        ...
        labs(title = "Recommended Portfolio (marginal benefit per card)", x = "", y = "Net Benefit") +
