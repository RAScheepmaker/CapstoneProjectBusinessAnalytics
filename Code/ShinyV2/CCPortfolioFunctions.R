...
get_budget <- function (income = "avg", budget_data) {
  ...
  # "avg" is accepted as a special input argument to return the average budget
  # from the Bureau of Labor Statistic's Consumer Expenditure Survey (2022),
  # corresponding to an average income of $94,003.
...
get_portfolio <- function (K = 4, eta = 0, theta = 0, cards_data, budget, 
                           verbose = TRUE) {
  ...
  # Benefits that should only be valued once (remaining 'benefit_credits' will
  # be valued per card and are assumed to be stackable)
  single_use_benefits <- c("benefit_globalentry", "benefit_lounge","benefit_clear")
  # Benefit values
  ge_value <- 20      # Global Entry / TSA precheck value in dollars per year
  ...
  # benefit_value <- matrix(0, nrow = length(cards_data[,1]), ncol = length(singele_use_benefits))
  benefit_value <- sweep(cards_data[ , single_use_benefits], 2, single_use_benefits_values, '*')
  # Initialize the category spend value matrix
  ...
  for (k in 1:K) {
    ...
    # Multiply all single use benefits with (1 - single_use_benefits) of max_ind to
    # to set them to zero after the first time they have been selected
    benefit_value <- sweep(benefit_value, 2, unname(unlist(1 - cards_data[max_ind, single_use_benefits])), "*")
